...
read -p "Instance name for /etc/odoo_deploy/<name>.env [$SERVICE_NAME]: " INSTANCE_NAME
INSTANCE_NAME=${INSTANCE_NAME:-$SERVICE_NAME}

read -p "Default DB name (for deploy/backup scripts) [$OE_USER]: " DB_NAME
...
# -------------------------------
# Deploy env file (for deploy/backup scripts)
# -------------------------------

echo "[8.5/9] Creating /etc/odoo_deploy env file..."

DEPLOY_ENV_DIR="/etc/odoo_deploy"
DEPLOY_ENV_FILE="${DEPLOY_ENV_DIR}/${INSTANCE_NAME}.env"

mkdir -p "$DEPLOY_ENV_DIR"

cat <<EOF >"$DEPLOY_ENV_FILE"
# Auto-generated by odoo_install.sh
OE_HOME=$OE_HOME
OE_USER=$OE_USER
BRANCH=$BRANCH
